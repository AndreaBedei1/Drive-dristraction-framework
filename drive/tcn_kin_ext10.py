...
Changes from ext9:
  1. Batch-level kin_pool zeroing removed.  ext9 contained
       if self.training and torch.rand(1).item() < 0.2:
           kin_pool = torch.zeros_like(kin_pool)
     torch.rand(1) draws ONE scalar per batch, so 20% of training steps erased
     the entire TCN representation for ALL samples simultaneously.  With phys
     scalars being near-uninformative (HR perm-importance ≈ 0.001), this
     effectively trained the head on noise 20% of the time.  Removed.
  2. arousal re-added to PHYS_COLS.  ext9 dropped arousal silently (PHYS_COLS
     was set to ["hr"] only) despite ext4 showing it discriminative at p=1.9e−4.
     The dataset contains the column; it is now included alongside hr.
     PHYS_SCALAR_D becomes 6 (mean, std, slope × 2 signals).
  3. pos_weight removed from BCE loss after SMOTE.  SMOTE already balances to
     ~60:40 neg:pos; applying pos_weight≈1.5 on top double-penalises negatives.
     Plain BCEWithLogitsLoss (no pos_weight) is correct for a SMOTE-balanced set.
  4. FiLM hidden dim reduced 64 → 16.  The FiLM generator conditioned a 6-d
     physiological input through 64 hidden units to modulate 64 kinematic channels.
     This over-parameterised mapping encouraged the network to learn spurious
     cross-driver correlations in a LOPO setting.  16-d hidden is proportionate.

Changes from ext8 (inherited):
  1. Spectral features dropped.
  2. Focal loss + WeightedRandomSampler replaced by plain BCE.
  3. Per-channel amplitude augmentation added.
...
Architecture:
  ...
  2. Physiological scalars injected at head:
     [mean(hr), std(hr), slope(hr), mean(arousal), std(arousal), slope(arousal)]
     computed from the raw window after LOO route renormalisation; scaled by a
     separate StandardScaler.  PHYS_SCALAR_D = 6.
  3. FiLM conditioning: 6-d phys → 16-d hidden → 128-d (γ, β) for KIN_D=64.
  4. Head: Linear(64+6, 64) → ReLU → Dropout(0.2) → Linear(64, 1).
...
SIGNAL_COLS  = ["hr", "arousal", "speed.x", "steeringWheelAngle", "steeringTorq", "acceleration.y"]
VEHICLE_COLS = ["speed.x", "steeringWheelAngle", "steeringTorq", "acceleration.y"]
PHYS_COLS    = ["hr", "arousal"]
...
PHYS_SCALAR_D = len(PHYS_COLS) * 3   # [mean, std, slope] per signal = 6
...
class KinTCN(nn.Module):
    ...
    Head input = KIN_D(64) + PHYS_SCALAR_D(6) = 70-d (spectral dropped in ext8).
    ...
    def __init__(self, n_kin_feats: int, phys_scalar_d: int, spectral_dim: int):
        ...
        # FiLM Generator: Physiology → Gamma & Beta (proportionate capacity)
        # 16-d hidden is sufficient for a 6-d phys input; 64-d overfit in LOPO.
        self.phys_film = nn.Sequential(
            nn.Linear(phys_scalar_d, 16),
            nn.LayerNorm(16),
            nn.ReLU(),
            nn.Linear(16, self.KIN_D * 2)
        )

        # Head receives: kin pool + raw phys scalars (direct bypass) + spectral
        head_in = self.KIN_D + phys_scalar_d + spectral_dim   # 64 + 6 + 0 = 70
        self.head = nn.Sequential(
            nn.Linear(head_in, 64),      # slightly larger hidden layer
            nn.ReLU(), 
            nn.Dropout(0.2), 
            nn.Linear(64, 1),
        )
    ...
    def forward(self, x_kin, x_phys_scalar, x_spec):
        ...
        # FiLM on the entire temporal feature map
        film_params = self.phys_film(x_phys_scalar)
        gamma, beta = torch.chunk(film_params, 2, dim=-1)
        gamma = gamma.unsqueeze(-1)
        beta  = beta.unsqueeze(-1)
        kin_seq = kin_seq * (1 + gamma) + beta
        
        kin_pool = self.kin_attn(kin_seq)

        # Direct bypass: concatenate raw phys scalars to the head
...
def _val_threshold(y_val, val_scores, mode="f1", target_recall=0.8):
    """
    Threshold selection based on Precision-Recall curve.

    Parameters
    ----------
    mode : str
        "f1"       → maximize F1 score (default)
        "recall"   → enforce minimum recall (uses target_recall)
    target_recall : float
        Minimum recall when mode="recall"
    ...
    precision, recall, thrs = precision_recall_curve(y_val, val_scores)

    # thrs has length = len(precision) - 1
    # align arrays
    ...
    if mode == "f1":
        ...
        if len(idx) == 0:
            return float(thrs[-1])  # fallback: most permissive
        return float(thrs[idx[-1]])
...
def train_kin_tcn(model, Xtr_k, Xtr_ps, Xtr_s, y_tr,
    ...
    # Plain BCE: SMOTE has already balanced the training set to ~60:40.
    # Applying pos_weight on top would double-penalise negatives.
...
def main():
    ...
    print(f"Phys scalars      : {PHYS_COLS}  →  [mean, std, slope] each = {PHYS_SCALAR_D}-d")
    ...
    print(f"Loss              : BCE (balanced by SMOTE; no focal, no pos_weight, no double-weighting)")
    ...
    for d in drivers:
        ...
        thresh_tcn  = _val_threshold(y_val_d, val_tcn_sc, mode="f1")
        thresh_tcnp = _val_threshold(y_val_d, val_tcnp_sc, mode="recall", target_recall=0.8)
    ...
    for fi, (model_i, Xk_i, Xps_i, Xs_i, y_i) in enumerate(
        ...
        for rep in range(N_PERM_REPEATS):
            ...
            # Permute physiological scalars.
            # Layout: [mean×C | std×C | slope×C], so signal pi occupies columns
            # [pi, pi+C, pi+2C] where C = len(PHYS_COLS).
            C_phys = len(PHYS_COLS)
            for pi, col in enumerate(PHYS_COLS):
                Xps_perm = Xps_i.copy()
                perm_idx = rng_pi.permutation(len(Xps_perm))
                phys_feat_cols = [pi, pi + C_phys, pi + 2 * C_phys]
                Xps_perm[:, phys_feat_cols] = \
                    Xps_perm[perm_idx, :][:, phys_feat_cols]
                perm_auc = safe_auc(y_i, torch.sigmoid(model_i(
                    torch.as_tensor(Xk_i).to(DEVICE),
