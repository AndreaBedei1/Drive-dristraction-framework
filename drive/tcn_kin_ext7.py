...
Changes from ext5:
  - LOOKBACK_S reduced from 90s to 45s.  Rationale: the kinematically informative
    period during a distraction is the driver's active response window, not the
    pre-distraction baseline.  90s includes substantial baseline noise; 45s focuses
    the model on the mid-to-late distraction period where control degradation is most
    predictive.  Shorter windows also increase the number of training samples per fold
    (same WINDOW_STEP=5, more windows fit per session).
...
def head_adapt(model_pop, Xte_k, Xte_ps, Xte_s, y_te,
    ...
    for _ in range(n_steps):
        opt.zero_grad()
        loss = focal_loss(model(Xk, Xps, Xs), ys, pos_weight=pos_w)
...
def main():
    ...
    print(f"Loss              : focal (γ=2) + pos_weight=neg/pos")
    ...
    for d in drivers:
        ...
        pw_smote = float((y_tr_train == 0).sum() / max((y_tr_train == 1).sum(), 1))
    ...
    out_path = OUT_DIR / "kin_tcn_ext7_results.json"
