...
def train_autoencoder(X_tr_normal):
    """Train only on normal (negative) windows."""
    dataset = DrivingDataset(X_tr_normal,
                              np.zeros(len(X_tr_normal)))
...
def lopo_autoencoder(X, y, pid):
    ...
    print(f"\nAE LOPO AUROC: {np.mean(aucs):.3f} "
          f"± {np.std(aucs):.3f}")
    print(f"AE LOPO AP:    {np.mean(aps):.3f} "
          f"± {np.std(aps):.3f}")
    return results
