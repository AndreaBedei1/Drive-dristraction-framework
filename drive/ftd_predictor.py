...
def full_nested_eval(H):
    ...
    for test_user in all_users:
        ...
        # cld = 1.0 active, exp(-t/H) recovery, 0.0 baseline: perfectly encodes
        # temporal position so the calibrator can follow the decay curve even when
        # the raw model assigns similar scores to 1s vs 6s post-distraction.
        # monotone_constraints ensures higher raw score => higher calibrated prob.
    ...
    # Uses adaptive quantile bins so the metric is stable even with rare positives.
