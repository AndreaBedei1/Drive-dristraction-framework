"""
tcn_kin_ext13_audited.py — Pure KinTCN driving risk score with K=5 ensemble + safe driver audit.

Changes from ext11:
  1. K=5 ensemble per fold.  Five KinTCN models are trained per LOPO fold
     using different random seeds (SEED ^ seed_d ^ k for k in 0..4).
     Their sigmoid outputs are averaged before Platt calibration.
     Rationale: with N~20 evaluable drivers and sparse positives, a single
     training run has high variance due to random weight initialisation and
     SMOTE sampling.  Averaging K independent runs reduces per-driver
     prediction variance without touching the data or evaluation protocol.
     No leakage: all K models are trained on the same training fold;
     Platt is fit on the same val fold scores (averaged across ensemble).
  2. Output filename updated to kin_tcn_ext13_results.json.
...
def main():
    ...
    print(f"SMOTE       : active={USE_SMOTE} k={SMOTE_K_NEIGHBORS} ratio {TARGET_RATIO}:{1-TARGET_RATIO} pos:neg")
    ...
    out_path = OUT_DIR / "kin_tcn_ext13_audited_results.json"
