...
print(f"  {'H (s)':<8} {'Nested AUC-PR':<16} {'Nested AUC-ROC':<16} {'Grid AUC-PR'}")
print("  " + "-" * 56)
...
for H_cand in top_candidates:
    ...
    print(f"  {H_cand:<8} {res['AUC-PR']:<16.4f} {res['AUC-ROC']:<16.4f} "
          f"{grid_aucpr:.4f}  {'← winner (so far)' if res == max(nested_results, key=lambda r: r['AUC-PR']) else ''}")

# Pick the winner
best_result = max(nested_results, key=lambda r: r['AUC-PR'])
...
print(f"  {'H':>4}  {'Grid AUC-PR':>12}  {'Nested AUC-PR':>14}  {'Nested AUC-ROC':>15}")
print("  " + "-" * 50)
for _, row in nested_summary.sort_values('nested_AUC_PR', ascending=False).iterrows():
    ...
    print(f"  {int(row['H']):>4}  {row['grid_AUC_PR']:>12.4f}  "
          f"{row['nested_AUC_PR']:>14.4f}  {row['nested_AUC_ROC']:>15.4f}{marker}")
print()
