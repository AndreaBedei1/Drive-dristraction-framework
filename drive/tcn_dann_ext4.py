...
def normalize_signals(df):
    ...
    for _, grp in df.groupby(["id", "route"]):
        ...
        # Per-session HR normalization: makes HR relative to each driver's
        # intra-session baseline (same treatment as vehicle signals).
        # Arousal is already baseline-relative; this brings HR in line.
        if "hr" in df.columns:
            mu  = grp["hr"].mean()
            sig = grp["hr"].std() + 1e-6
            df.loc[idx, "hr"] = (grp["hr"] - mu) / sig
    return df
...
def smote_oversample(X_p, X_k, X_s, y, k=SMOTE_K_NEIGHBORS, rng=None):
    """
    SMOTE oversampling applied to the *flattened engineered features*.

    Operates on the flat feature representation (N, T*C) to synthesise
    interpolated minority-class windows, then reshapes back to (N, T, C).
    Applied to training data only — never to validation or test sets.

    Parameters
    ----------
    X_p, X_k, X_s : ndarray — (N, T, C_p), (N, T, C_k), (N, S) scaled features
    y              : ndarray — (N,) binary labels
    k              : int     — number of nearest neighbours for SMOTE

    Returns
    -------
    X_p, X_k, X_s, y — augmented arrays with synthetic minority samples appended
    """
    if rng is None:
    ...
    if n_pos == 0 or n_pos >= n_neg:
        return X_p, X_k, X_s, y   # already balanced or no positives

    n_synthetic = n_neg - n_pos

    # Flatten each modality for nearest-neighbour computation
    T_p, C_p = X_p.shape[1], X_p.shape[2]
    T_k, C_k = X_k.shape[1], X_k.shape[2]
    S        = X_s.shape[1]

    Xp_flat = X_p[pos_idx].reshape(n_pos, -1)   # (n_pos, T*C_p)
    Xk_flat = X_k[pos_idx].reshape(n_pos, -1)
    Xs_flat = X_s[pos_idx]                       # (n_pos, S)

    # Concatenate all modalities for neighbour search
    X_concat = np.concatenate([Xp_flat, Xk_flat, Xs_flat], axis=1)  # (n_pos, D)

    # Compute pairwise distances among positives
    k_eff = min(k, n_pos - 1)
    if k_eff < 1:
        # Too few positives for SMOTE — fall back to random duplication
        dup_idx = rng.choice(pos_idx, n_synthetic, replace=True)
        return (np.concatenate([X_p, X_p[dup_idx]]),
                np.concatenate([X_k, X_k[dup_idx]]),
                np.concatenate([X_s, X_s[dup_idx]]),
    ...
    syn_p = np.zeros((n_synthetic, T_p, C_p), dtype=X_p.dtype)
    syn_k = np.zeros((n_synthetic, T_k, C_k), dtype=X_k.dtype)
    syn_s = np.zeros((n_synthetic, S),         dtype=X_s.dtype)
    ...
    for i in range(n_synthetic):
        ...
        syn_p[i] = X_p[pos_idx[anchor]] + lam * (X_p[pos_idx[nn]] - X_p[pos_idx[anchor]])
        syn_k[i] = X_k[pos_idx[anchor]] + lam * (X_k[pos_idx[nn]] - X_k[pos_idx[anchor]])
        syn_s[i] = X_s[pos_idx[anchor]] + lam * (X_s[pos_idx[nn]] - X_s[pos_idx[anchor]])

    X_p_aug = np.concatenate([X_p, syn_p], axis=0)
    X_k_aug = np.concatenate([X_k, syn_k], axis=0)
    X_s_aug = np.concatenate([X_s, syn_s], axis=0)
    y_aug   = np.concatenate([y,   np.ones(n_synthetic, dtype=y.dtype)])

    return X_p_aug, X_k_aug, X_s_aug, y_aug
...
class DANNDualBranchTCN(nn.Module):
    ...
    def __init__(self, n_phys_feats: int, n_kin_feats: int):
        ...
        # Head: fusion(96) + spectral(18) → 114 → 48 → 1
        ...
        self._last_phys_logits: torch.Tensor | None = None
...
def train_dann_tcn(model, discriminator,
    ...
    for epoch in range(EPOCHS):
        ...
        for xp, xk, xs, yb, sid in loader:
            ...
            aux_phys_loss  = focal_loss(model._last_phys_logits, yb, pos_weight=pos_weight)
...
def main():
    ...
    print(f"Normalisation     : vehicle + HR z-scored per session; arousal absolute (already baseline-relative)")
