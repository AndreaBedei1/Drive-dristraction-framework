"""
tcnKin_60.py — KinTCN ablation: LOOKBACK=60s, TCN d=1,2,4,8,16,32 (RF≈127).
...
def window_baseline_feats(X_raw):
    """Mean | std | max per signal — for XGB baseline."""
...
class KinTCN(nn.Module):
    ...
    ResBlocks d=1,2,4,8,16,32 → RF ≈ 127 timesteps → TemporalAttention → head.
...
def main():
    ...
    hdr = (f"{'Driver':<10} | {'N_win':>5} {'PosR%':>6} | "
           f"{'LSTM':>7} {'XGB':>7} {'KinTCN':>7} {'KinTCN-NA':>9} {'KinTCN-NA+C':>12} {'KinTCN+Cal':>11}")
    print(hdr); print("-" * len(hdr))

    per_driver_results = []
    pool_y, pool_tcn, pool_cal = [], [], []
    pool_lstm, pool_xgb, pool_noatt, pool_noatt_cal = [], [], [], []
    ...
    for d in drivers:
        ...
        noatt_scores_list = []; noatt_val_list = []
        for k in range(ENSEMBLE_K):
            ...
            with torch.no_grad():
                ...
                noatt_val_list.append(torch.sigmoid(noatt_k(Xval_t)).cpu().numpy())
        noatt_scores     = np.mean(noatt_scores_list, axis=0)
        noatt_val_scores = np.mean(noatt_val_list,    axis=0)

        # ── LSTM baseline ─────────────────────────────────────────────────────
        lstm_scores_list = []; lstm_val_list = []
        for k in range(ENSEMBLE_K):
            kseed_l = SEED ^ seed_d ^ (k * 0x3B7F)
            torch.manual_seed(kseed_l); torch.cuda.manual_seed_all(kseed_l)
            lstm_k = KinLSTM(n_kin_feat).to(DEVICE)
            lstm_k = train_kin_tcn(lstm_k, Xtr_k_sc_all, y_tr_aug, sc_tr_aug,
                                   Xval_k_sc, y_val_d)
            lstm_k.eval()
            with torch.no_grad():
                lstm_scores_list.append(torch.sigmoid(lstm_k(Xte_t)).cpu().numpy())
                lstm_val_list.append(torch.sigmoid(lstm_k(Xval_t)).cpu().numpy())
        lstm_scores = np.mean(lstm_scores_list, axis=0)

        # Val-set Platt calibration
        cal_scores,       platt_lr = val_platt_calibrate(val_scores,      y_val_d, tcn_scores)
        noatt_cal_scores, _        = val_platt_calibrate(noatt_val_scores, y_val_d, noatt_scores)
        ...
        auc_noatt_cal = _nan_or(safe_auc(y_te, noatt_cal_scores))
        auc_cal       = _nan_or(safe_auc(y_te, cal_scores))

        print(f"{d:<10} | {len(y_te):>5} {100*y_te.mean():>6.1f}% | "
              f"{auc_lstm:>7.4f} {auc_xgb:>7.4f} {auc_tcn:>7.4f} "
              f"{auc_noatt:>9.4f} {auc_noatt_cal:>12.4f} {auc_cal:>11.4f}")
        ...
        pool_noatt.append(noatt_scores); pool_noatt_cal.append(noatt_cal_scores)
        ...
        per_driver_results.append({
            ...
            "auc_tcn": auc_tcn, "auc_noatt": auc_noatt,
            "auc_noatt_cal": auc_noatt_cal, "auc_cal": auc_cal,
        })
    ...
    all_noatt_cal = np.concatenate(pool_noatt_cal)
    ...
    drv_aucs_noatt_cal= [v for v in (safe_auc(y, s) for y, s in zip(pool_y, pool_noatt_cal)) if v is not None]
    ...
    metrics["kin_noatt"]    = _print_model("KinTCN (no attn, raw)", all_noatt,     all_y, drv_aucs_noatt)
    metrics["kin_noatt_cal"]= _print_model("KinTCN (no attn)+Cal",  all_noatt_cal, all_y, drv_aucs_noatt_cal)
    ...
    pred_rows = [
        {
            ...
            "kin_noatt_cal": noatt_cal.tolist(),
            "kin_cal":       cal.tolist(),
        }
        for rec, y, lstm, xgb, tcn, noatt, noatt_cal, cal in zip(
            per_driver_results, pool_y, pool_lstm, pool_xgb,
            pool_tcn, pool_noatt, pool_noatt_cal, pool_cal)
    ]
