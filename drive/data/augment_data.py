...
Design principles
─────────────────
Physiology   : new runs generated by resampling real-world sensor sessions from
               sensor_driver_arousal.json (69 sessions, ~1037 s each).  Each
               session is z-scored and remapped to the participant's
               (target_mean, ar_range) profile, preserving authentic spike
               dynamics (rise ≈5.5 s, fall ≈3.6 s, ~25 spikes/session).
               Ornstein-Uhlenbeck is kept as a fallback if the sensor file
               is unavailable.
Error timing : event-level — each distraction event independently draws a
               Bernoulli(p) label where p ∝ local pre-distraction arousal
               (5 s window) scaled by participant pattern (positive/negative, k).
               Triggered distractions place one error via Exp(τ) in the hangover
               zone; τ ∈ [2.0, 5.5] s per participant.
               p_base = 0.50: ~50 % of distractions trigger a post-distraction error.
...
# ── real-world session pool ───────────────────────────────────────────────────
# relab+unibo_dataset.csv (44 routes, 2 participants).
# The sensor_driver_arousal.json data is already included in this CSV.

_RELAB_CSV = os.path.join(os.path.dirname(DATA_DIR), 'relab+unibo_dataset.csv')


def _load_relab_sessions() -> list:
    """Extract per-route arousal/HR sequences from relab+unibo_dataset.csv.

    Each route contributes two sequences to the pool:
      - full route (all safe_driving values) — captures realistic dynamics
      - safe_driving==1 sub-sequence only   — calmer baseline physiology
    Both are used equally when sampling for synthetic physiology generation.
    """
    if not os.path.isfile(_RELAB_CSV):
        return []
    df = pd.read_csv(_RELAB_CSV, usecols=['id', 'route', 'Timestamp',
                                           'hr', 'arousal', 'safe_driving'])
    df['Timestamp'] = pd.to_datetime(df['Timestamp'], utc=True, errors='coerce')
    df = df.dropna(subset=['Timestamp', 'arousal', 'hr'])
    df = df.sort_values(['route', 'id', 'Timestamp'])

    sessions = []
    for (route, rid), grp in df.groupby(['route', 'id']):
        ar = grp['arousal'].values.astype(float)
        hr = grp['hr'].values.astype(float)
        if len(ar) >= 20:
            sessions.append({'ar': ar, 'hr': hr})

        # also add the safe-only sub-sequence if long enough
        safe = grp[grp['safe_driving'] == 1]
        if len(safe) >= 20:
            sessions.append({'ar': safe['arousal'].values.astype(float),
                             'hr': safe['hr'].values.astype(float)})
    return sessions


# Build the session pool at import time.
_ALL_SESSIONS: list = _load_relab_sessions()


def _resample(values: np.ndarray, n_out: int) -> np.ndarray:
    """Linear interpolation of a 1-D array to n_out points."""
    if len(values) == n_out:
        return values.copy()
    x_in  = np.linspace(0.0, 1.0, len(values))
    x_out = np.linspace(0.0, 1.0, n_out)
    return np.interp(x_out, x_in, values)


def _fill_nans(values: np.ndarray) -> np.ndarray:
    """Linear interpolation over NaNs in a 1-D array."""
    nans = np.isnan(values)
    if not nans.any():
        return values
    idx = np.arange(len(values))
    return np.interp(idx, idx[~nans], values[~nans])
...
def estimate_dt(tl: pd.DataFrame) -> float:
    """Mean sampling interval in seconds from timeline timestamps."""
    ts = pd.to_datetime(tl['timestamp'], errors='coerce').dropna()
    if len(ts) < 2:
        return 0.5
    return float((ts.iloc[-1] - ts.iloc[0]).total_seconds() / (len(ts) - 1))


# ── OU fallbacks (used when session pool is empty) ────────────────────────────

def make_arousal_ar1(target_mean: float, ar_range: float, n: int,
                     dt_sec: float = 0.5) -> np.ndarray:
    """Ornstein-Uhlenbeck fallback — time constant ≈ 50 s."""
    ar_std = ar_range / 4
    theta  = 0.020
    eps    = ar_std * 0.10 * np.sqrt(dt_sec)
    x      = np.zeros(n)
    x[0]   = np.clip(target_mean + np.random.normal(0, ar_std * 0.5), 0.02, 0.99)
    for i in range(1, n):
        x[i] = x[i-1] + theta * dt_sec * (target_mean - x[i-1]) + np.random.normal(0, eps)
    return np.clip(x, 0.02, 0.99)


def make_hr_ar1(target_mean: float, hr_std: float, n: int,
                dt_sec: float = 0.5) -> np.ndarray:
    """Ornstein-Uhlenbeck HR fallback — time constant ≈ 67 s."""
    theta = 0.015
    eps   = hr_std * 0.12 * np.sqrt(dt_sec)
    x     = np.zeros(n)
    x[0]  = np.clip(target_mean + np.random.normal(0, hr_std * 0.5), 35, 130)
    for i in range(1, n):
        x[i] = x[i-1] + theta * dt_sec * (target_mean - x[i-1]) + np.random.normal(0, eps)
    return np.clip(np.round(x), 35, 130)


# ── sensor-based physiology (primary path) ────────────────────────────────────

def make_arousal_from_sensor(target_mean: float, ar_range: float, n: int,
                              dt_sec: float = 0.5) -> np.ndarray:
    """
    Sample a random real-world session, resample to n points, then z-score
    and remap to (target_mean, ar_range/4) preserving authentic dynamics.
    Falls back to OU if no sessions are available.
    """
    if not _ALL_SESSIONS:
        return make_arousal_ar1(target_mean, ar_range, n, dt_sec)
    sess    = _ALL_SESSIONS[np.random.randint(len(_ALL_SESSIONS))]
    ar_raw  = _fill_nans(sess['ar'].copy())
    ar_res  = _resample(ar_raw, n)
    mu, sig = ar_res.mean(), ar_res.std()
    if sig < 1e-6:
        sig = 1e-6
    ar_new  = (ar_res - mu) / sig * (ar_range / 4.0) + target_mean
    return np.clip(ar_new, 0.02, 0.99)


def make_hr_from_sensor(target_mean: float, hr_std: float, n: int,
                         dt_sec: float = 0.5) -> np.ndarray:
    """
    Sample a random real-world session, resample to n points, then z-score
    and remap to (target_mean, hr_std) preserving authentic HR dynamics.
    Falls back to OU if no sessions are available.
    """
    if not _ALL_SESSIONS:
        return make_hr_ar1(target_mean, hr_std, n, dt_sec)
    sess    = _ALL_SESSIONS[np.random.randint(len(_ALL_SESSIONS))]
    hr_raw  = _fill_nans(sess['hr'].astype(float).copy())
    hr_res  = _resample(hr_raw, n)
    mu, sig = hr_res.mean(), hr_res.std()
    if sig < 1e-6:
        sig = max(hr_std * 0.5, 1.0)
    hr_new  = (hr_res - mu) / sig * hr_std + target_mean
    return np.clip(np.round(hr_new), 35, 130)


# ── event-level error model ───────────────────────────────────────────────────

def per_distraction_labels(dis_df: pd.DataFrame, tl: pd.DataFrame,
                            pid_num: int, run_mean_ar: float,
                            window_s: float = 5.0) -> list:
    """
    For each distraction event, draw Bernoulli(p) where p depends on the
    mean arousal in [ts_start - window_s, ts_start].

    Positive pattern : p = p_base × (1 + k × z)   high arousal → more errors
    Negative pattern : p = p_base × (1 − k × z)   high arousal → fewer errors

    Returns list of int (0 or 1), one per distraction row.
    """
    ...
    ts_tl   = pd.to_datetime(tl['timestamp'], errors='coerce')
    arousal = tl['arousal'].values

    labels = []
    for _, row in dis_df.iterrows():
        ts_s   = pd.to_datetime(row['timestamp_start'], errors='coerce')
        win_lo = ts_s - pd.Timedelta(seconds=window_s)
        mask   = (ts_tl >= win_lo) & (ts_tl < ts_s)
        vals   = arousal[mask.values]
        local_ar = float(np.nanmean(vals)) if len(vals) > 0 else run_mean_ar
        if np.isnan(local_ar):
            local_ar = run_mean_ar
        z = float(np.clip((local_ar - ar_mean) / max(ar_std, 0.04), -2, 2))
        p = float(np.clip(p_base * (1.0 + sign * k * z), 0.02, 0.98))
        labels.append(int(np.random.binomial(1, p)))
    return labels


def inject_errors_event_level(tl: pd.DataFrame,
                               per_dis_labels: list,
                               dis_windows: list,
                               t_start: datetime, t_end: datetime,
                               tau: float,
                               n_background: int = 2) -> tuple:
    """
    Place one error per triggered distraction in the hangover zone via Exp(τ),
    capped at the next distraction start or +13 s.
    Also place n_background uniform errors across the run.
    """
    tl = tl.copy()
    ...
    for i, (label, win) in enumerate(zip(per_dis_labels, dis_windows)):
        if label == 0:
            continue
        ts_s, ts_e = win
        nxt = dis_windows[i + 1][0] if i + 1 < len(dis_windows) else t_end
        cap = min(
            (nxt - ts_e).total_seconds(),
            (t_end - ts_e).total_seconds(),
            13.0
        )
        if cap <= 0.2:
            continue
        ...
        for _ in range(50):
            ...
            if t_off < cap:
                err_ts_list.append(ts_e + timedelta(seconds=t_off))
                placed = True
                break
        if not placed:
            err_ts_list.append(ts_e + timedelta(seconds=min(tau, cap * 0.8)))

    for _ in range(n_background):
        off = np.random.uniform(run_dur * 0.05, run_dur * 0.95)
        err_ts_list.append(t_start + timedelta(seconds=off))

    n_err = len(err_ts_list)
    if n_err == 0:
        return tl, []
...
def process_dist_run(pid_num, run_id, run_idx, n_total,
    ...
    is_existing=True  → keep real arousal/hr/positions; re-derive errors from local arousal
    is_existing=False → generate AR(1) physiology; derive errors from local arousal
    """
    hr_m, hr_s, ar_mean, ar_range = PROFILES[pid_num]
    ...
    if not is_existing:
        ...
        dt_sec    = estimate_dt(tmpl_tl)
        n         = len(tl)
        tl['arousal']            = make_arousal_from_sensor(target_ar, ar_range, n, dt_sec)
        tl['hr_bpm']             = make_hr_from_sensor(hr_m, hr_s, n, dt_sec)
    ...
    # event-level labels: Bernoulli(p) per distraction using local pre-distraction arousal
    per_dis = per_distraction_labels(dis_df, tl, pid_num, target_ar, window_s=5.0)
    dis_windows = [
        (pd.to_datetime(r['timestamp_start'], errors='coerce'),
         pd.to_datetime(r['timestamp_end'],   errors='coerce'))
        for _, r in dis_df.iterrows()
    ]
    n_background = max(1, int(round(len(dis_df) * 0.10)))
    tl, err_rows = inject_errors_event_level(
        tl, per_dis, dis_windows, t_start, t_end, TAU[pid_num], n_background
    )
...
def process_base_run(pid_num, run_id, run_idx, n_total,
    ...
    if not is_existing:
        ...
        n         = len(tl)
        tl['arousal']            = make_arousal_from_sensor(target_ar, ar_range, n, dt_sec)
        tl['hr_bpm']             = make_hr_from_sensor(hr_m, hr_s, n, dt_sec).astype(int)
        tl['timestamp']          = _reanchor(tmpl_tl['timestamp'], base_dt)
        tl['model_timestamp']    = _reanchor(tmpl_tl['model_timestamp'], base_dt, iso=True)
        tl['emotion_timestamp']  = _reanchor(tmpl_tl['emotion_timestamp'], base_dt, iso=True)
        tl['arousal_timestamp_ms'] = _reanchor(tmpl_tl['arousal_timestamp_ms'], base_dt, iso=True)
    ...
    n_err = max(0, int(round(BASE_BASE_ERRORS + np.random.normal(0, 1.0))))
