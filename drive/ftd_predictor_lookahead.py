...
def compute_user_baselines(users_set: Set) -> Tuple[Dict, float]:
    """Error rate from baseline runs only."""
...
def generate_samples(
    ...
    """
    Generate per‑second samples for users in users_set.
    Target = 1 if any error occurs in [current_ts, current_ts + T).
    Features are computed using only data available up to current_ts.
    """
    rows = []
    dist_sel = (
    ...
    for _, row in dist_sel.iterrows():
        ...
        # Hangover period limited by next distraction or H
        next_rows  = dist_sel[
        ...
        for offset_sec in range(num_bins):
            ...
            bin_end   = bin_start + pd.Timedelta(seconds=1)   # not used for target
            ...
            target = 0
            idx = bisect_left(errs, bin_start)
            if idx < len(errs) and errs[idx] < bin_start + pd.Timedelta(seconds=T):
                target = 1
            ...
            rows.append({
                ...
                'time_since_dist_end':   time_since_end,
...
train_aro_bl, train_aro_global = compute_user_arousal_baseline(train_users, wbs_tr_f)
train_hr_bl,  train_hr_global  = compute_user_hr_baseline(train_users, wbs_tr_f)
...
# ---- Calibration data (using training baselines) ----
