...
# ------------------------------------------------------------------------------
# Feature set for ablation: distraction‑only features (no physiology)
# ------------------------------------------------------------------------------
...
# For backward compatibility – kept but not used as HMM features
...
ROLL_SOURCE_COLS = ["model_prob"]
ROLL_FEATURE_COLS = [f"{col}_diff1_abs" for col in ROLL_SOURCE_COLS]
        ...
        for i in range(len(wins)):
            ...
            for off in range(n_bins):
                ...
                rows.append({
                    ...
                    # within_distraction kept as metadata, NOT in FEATURE_COLS
...
def add_causal_session_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["model_prob_diff1_abs"] = np.abs(grp["model_prob"].diff().fillna(0.0))
...
# ------------------------------------------------------------------------------
class ImpairmentHMM:
    def __init__(self, n_states=3, covariance_type="diag", n_iter=150, tol=1e-4,
                 random_state=RANDOM_SEED, constrain_backward=False,
                 transmat_prior_scale=15.0, variance_floor=1e-3):
    ...
    def _build_transition_prior(self, scale):
        K = self.n_states
    ...
    def _zero_backward_arcs(self, A):
        A = A.copy()
        for i in range(1, self.n_states):
            A[i, :i] = BWD_FLOOR
        return A / A.sum(axis=1, keepdims=True).clip(1e-12)

    def _supervised_init(self, X, labels):
        """Initialise HMM parameters from label-guided cluster statistics.
        
        Returns a GaussianHMM with parameters set but NOT yet fitted via EM.
        The caller's loop will run all EM iterations.
        """
    ...
    def predict_posteriors(self, X, bounds):
    ...
    def predict_viterbi(self, X, bounds):
        if self.model_ is None:
            raise RuntimeError("Not fitted")
...
# ------------------------------------------------------------------------------
class PICLabeler:
    def __init__(self, n_bins=30, random_state=RANDOM_SEED):
        self.n_bins = n_bins
        self.random_state = random_state
        self.scaler_ = None
        self.coef_ = None
    ...
    def fit(self, df_ca, train_p_error_baseline, lookahead_t, n_states,
            feature_cols=None, min_samples_per_bin=10):

        if feature_cols is None:
            feature_cols = [c for c in BASE_PHYSIO_FEATURES if c in df_ca.columns]
        if not feature_cols:
            raise ValueError("No features found in calibration DataFrame.")
        self.feature_cols_ = feature_cols
        self.n_states_ = n_states

        baseline_target_prob = 1.0 - np.exp(-train_p_error_baseline * lookahead_t)
        LOG.info("PICLabeler: baseline_target_prob=%.4f (λ=%.4f /s, T=%d s)",
        ...
        pos_rate = y_ca.mean()
        LOG.info("PICLabeler: fitted on %d CAL samples (%.1f%% positive), %d features",
                 len(y_ca), 100.0 * pos_rate, len(feature_cols))
        LOG.info("PICLabeler top-3 weights: %s",
                 sorted(zip(feature_cols, self.coef_), key=lambda x: abs(x[1]), reverse=True)[:3])

        # Thresholds derived from calibration PIC score distribution
        ...
        LOG.info("PICLabeler thresholds (high → low): %s",
    ...
    def predict_pic(self, df):
        if self.coef_ is None:
            raise RuntimeError("PICLabeler not fitted.")
        X_scaled = self.scaler_.transform(df[self.feature_cols_].astype(float).values)
        return self._pic(X_scaled)

    def _pic(self, X_scaled):
        logit = X_scaled @ self.coef_ + self.intercept_
        return 1.0 / (1.0 + np.exp(-logit))

    def _labels_from_pic(self, pic):
        # thresholds is length (K-1), sorted HIGH → LOW.
        # Walk every threshold in descending order so any K is handled correctly.
        thresholds = self.pic_thresholds_   # length = n_states - 1
        K = self.n_states_
        labels = np.full(len(pic), K - 1, dtype=int)
        for i, thr in enumerate(thresholds):
            labels[pic <= thr] = K - 2 - i
        return labels
...
def run_pipeline(args):
    ...
    LOG.info("Label mode: PIC (distraction-only, cal-fitted to avoid circularity)")
    pic_labeler = PICLabeler(
        n_bins=getattr(args, "pic_n_bins", 30),
        random_state=args.seed,
    )
    pic_labeler.fit(
        df_ca,                              # ← cal only (was df_tr, df_ca)
        train_p_error_baseline=train_p_error_baseline,
        lookahead_t=T,
        n_states=args.n_states,
        min_samples_per_bin=10,
    )
    lbl_tr = pic_labeler.predict(df_tr)    # out-of-sample labels for HMM supervision
    ...
    LOG.info("Using %d distraction-only features: %s", len(available), available)
    ...
    # Risk weights learned on calibration set posteriors
    ...
    LOG.info(
        "Test set (event-level): AUC-ROC = %.4f  95%%CI [%.4f, %.4f]"
        "  AP = %.4f",
        eval_risk["auc_roc"],
    ...
    joblib.dump(artifact, out_dir / "impairment_hmm_ablation.joblib")
    ...
    result = {
        ...
        # Per-sample (biased, kept for comparison)
...
def build_arg_parser():
    p = argparse.ArgumentParser(
        description="Driver Impairment HMM with PIC labels (distraction-only ablation)"
    )
    p.add_argument("--data-path", default="data", help="Path to data directory")
    p.add_argument("--output-dir", default="result_ablation/")
