...
N_ECE_BINS = 10   # adaptive quantile bins for ECE (10 deciles)
# ECE weight in H selection score: auc_pr - ECE_WEIGHT * ece_label
# ECE is computed against actual binary labels from test fold (honest).
# The old cal_mae (vs smoothed GT rates) is kept for diagnostics only.
...
# ── Calibrator (3-D: raw_score x cognitive_load_decay x time_since_last_dist → GT error rate) ─────────
# Using a small XGBoost regressor instead of 1-D IsotonicRegression.
# The 1-D calibrator maps f(raw_score) → GT_rate. Because the raw model assigns
# similar scores to, e.g., 1s vs 6s post-distraction, isotonic regression cannot
# distinguish them and outputs a flat line in the temporal-decay plot.
# Adding cognitive_load_decay = exp(-t/H) as a second input (1.0 if active, 0.0
# if baseline) lets the calibrator follow the true temporal decay curve.
# monotone_constraints=(1, 0): raw_score must be monotone-increasing (a higher
# raw score can never yield a lower calibrated probability); decay is unconstrained.
CAL_XGB_PARAMS = dict(
    n_estimators       = 200,
    max_depth          = 3,
    learning_rate      = 0.05,
    ...
    verbosity          = 0,
    random_state       = RANDOM_SEED,
    monotone_constraints = (1, 0, 0),   # (raw_score ↑, cld unconstrained, time_since unconstrained)
)

# ======================================================================
# UPDATED FEATURE COLS – added new physiological dynamics features
# ======================================================================
FEATURE_COLS = [
    # Core temporal state
    'distraction_active',
    ...
    'emotion_label_enc',
    'arousal_deviation',          
    # NEW: arousal change during the most recent distraction window
    'arousal_delta',
    # NEW: arousal level at the end of the most recent completed window
    'arousal_at_window_end',
    # NEW: binary indicator if emotion label changed during the window
    'emotion_changed',
    # NEW: heart rate (if available; otherwise 0)
    'hr_bpm',
    # NEW: quadratic terms for arousal and emotion
    'model_prob_sq',
    'emotion_prob_sq',
...
def compute_gt_profiles(user_ids_set=None):
    ...
    outside = err_subset[~err_subset.apply(is_error_inside_distraction, axis=1).fillna(False)].copy()
...
def build_positives(H, user_ids=None, arousal_bl=None, arousal_med=None):
    ...
    for _, err in errors_dist.iterrows():
        ...
        # ======================================================================
        # NEW FEATURES: arousal_delta, arousal_at_window_end, emotion_changed, hr_bpm, quadratic terms
        # ======================================================================
        # Initialize defaults (for baseline/hangover)
        arousal_delta = 0.0
        ...
        if dist_active == 1:
            # Error occurred inside a distraction window – find the window
            key = (uid, rid)
            wins = windows_by_session[key]
            # Find the specific window containing ts
            ...
            if 'hr_bpm_start' in win:
                hr_bpm = win['hr_bpm_start']   # use start value as representative
            ...
            # Error occurred after a window ended – find the most recent completed window
            key = (uid, rid)
            if key in windows_by_session:
                ...
                if not prev.empty:
                    ...
                    # For post‑distraction errors, arousal_delta is not defined; keep 0
                    # emotion_changed is also not defined; keep 0

        # Quadratic terms
        m_prob_sq = m_prob * m_prob
        e_prob_sq = err['emotion_prob'] * err['emotion_prob']
        aro_dev_sq = (m_prob - u_aro) ** 2
        ...
        rows.append({
            ...
            'hr_bpm':                  hr_bpm,
            'model_prob_sq':           m_prob_sq,
            'emotion_prob_sq':         e_prob_sq,
            'arousal_deviation_sq':    aro_dev_sq,
            # Long-run context
...
def build_negatives(H, sample_every=NEG_SAMPLE_EVERY, user_ids=None, arousal_bl=None, arousal_med=None):
    ...
    for (uid, rid), wins in windows_by_session.items():
        ...
        for w_idx, win in wins_s.iterrows():
            ...
            # ======================================================================
            # NEW FEATURES for inside windows
            # ======================================================================
            if 'arousal_start' in win and 'arousal_end' in win:
                arousal_delta = win['arousal_end'] - win['arousal_start']
                arousal_at_window_end = win['arousal_end']
            ...
            for offset in np.arange(0, dur, sample_every):
                ...
                if ts.floor('s') in err_set:
                    continue
                # Quadratic terms (use the window's start values)
                m_prob_sq = m_prob_w * m_prob_w
                e_prob_sq = win['emotion_prob_start'] * win['emotion_prob_start']
                aro_dev_sq = (m_prob_w - u_aro) ** 2
        ...
        for i in range(len(wins_s) - 1):
            ...
            # ======================================================================
            # NEW FEATURES for inter-window gaps (use end-of-window values)
            # ======================================================================
            if 'arousal_end' in win_state:
                arousal_at_window_end = win_state['arousal_end']
            else:
                arousal_at_window_end = 0.0
            if 'hr_bpm_end' in win_state:
                hr_bpm = win_state['hr_bpm_end']
            else:
                hr_bpm = 0.0
            # arousal_delta and emotion_changed are not defined for gaps; keep 0
            arousal_delta = 0.0
            emotion_changed = 0
            ...
            for offset in np.arange(0, gap_len, sample_every):
                ...
                # Quadratic terms
                m_prob_sq = m_prob_g * m_prob_g
                e_prob_sq = win_state['emotion_prob_end'] * win_state['emotion_prob_end']
                aro_dev_sq = (m_prob_g - u_aro) ** 2
...
def build_baseline_negatives(sample_every=NEG_SAMPLE_EVERY,
                              ...
                              default_hr_bpm=None,   # NEW: default heart rate
                              user_ids=None,
    ...
    for _, row in driving_base.iterrows():
        ...
        hr_bpm       = default_hr_bpm if default_hr_bpm is not None else 0.0
        for offset in np.arange(0, run_duration, sample_every):
            # Quadratic terms
            m_prob_sq = m_prob * m_prob
            e_prob_sq = default_emotion_prob * default_emotion_prob if default_emotion_prob is not None else 0.25
            aro_dev_sq = (m_prob - u_aro) ** 2
            ...
            rows.append({
                ...
                'arousal_deviation':      m_prob - u_aro,
                # NEW FEATURES – all zero for baseline
                'arousal_delta':           0.0,
                'arousal_at_window_end':   0.0,
                'emotion_changed':         0,
                'hr_bpm':                  hr_bpm,
...
def compute_fold_baseline_rate(user_ids_set):
    # Compute the baseline error rate (errors/second) restricted to training users.
    # All levels are fold-isolated -- p_baseline_global and user_baselines (which
    # contains entries filled with the global rate for users without baseline data)
    # are deliberately NOT used here.
    #
    # Level 1: direct aggregate from raw data (almost always taken).
    # Level 2: per-user rates from raw data, averaged (only if level 1 gives 0/0).
    # Level 3: return 0.0 -- unreachable in a complete dataset.
    ...
    # Level 2: per-user rates from raw data only
    ...
    if len(valid) > 0:
        return float(np.mean(per_user_errs[valid.index].values /
                             valid.values))

    # Level 3: no data for any training user (unreachable with complete data)
    return 0.0

def compute_ece(y_true, y_prob, n_bins=N_ECE_BINS):
    # Expected Calibration Error against actual binary labels.
    # Uses adaptive (quantile) binning so all bins are equally populated --
    # critical for this problem where calibrated probs cluster in 0.01-0.12.
    # ECE = sum_b (|mean_pred_b - frac_pos_b| * n_b) / N
    y_true = np.asarray(y_true)
    y_prob = np.asarray(y_prob)
    if len(y_true) == 0 or y_true.sum() == 0:
        return np.nan   # no positive labels -> ECE undefined
    # Quantile bin edges from the predicted probabilities
    quantiles = np.linspace(0, 100, n_bins + 1)
    edges = np.percentile(y_prob, quantiles)
    edges[0]  = 0.0
    edges[-1] = 1.0 + 1e-9   # ensure last sample is included
    ece   = 0.0
    total = len(y_true)
...
def full_nested_eval(H):
    ...
    # for ECE: collect actual binary labels paired with calibrated probs
    ...
    for test_user in all_users:
        ...
        if safe_mask.sum() > 0:
            ...
            # NEW: default heart rate from safe seconds
            def_hr  = train_df.loc[safe_mask, 'hr_bpm'].median() if 'hr_bpm' in train_df.columns else 0.5
        else:
            def_mp  = 0.5
        ...
        base_defaults = dict(default_model_prob=def_mp, default_model_pred_enc=def_mpe,
                             ...
                             default_hr_bpm=def_hr)

        base_neg_tr = build_baseline_negatives(**base_defaults, user_ids=train_users, **fold_kw)
        ...
        # Fit 3-D calibrator: (raw_score, cognitive_load_decay, time_since_last_dist) -> fold GT rates.
        ...
        cal_X_tr = np.column_stack([raw_tr, cld_tr, train_full['time_since_last_dist'].values])
        iso = xgb.XGBRegressor(**CAL_XGB_PARAMS)
        iso.fit(cal_X_tr, gt_tr)
        ...
        base_neg_te = build_baseline_negatives(**base_defaults, user_ids={test_user}, **fold_kw)
        ...
        cal_X_te = np.column_stack([raw_te, cld_te, test_full['time_since_last_dist'].values])
        cal_te   = np.clip(iso.predict(cal_X_te), 0.0, 1.0)
        ...
        # ECE accumulation: actual binary labels vs calibrated probabilities
    ...
    # cal_mae vs smoothed GT rates: kept for diagnostics only.
    # Comparing calibrated outputs to training-derived rate estimates is
    # optimistic: the calibrator was trained to reproduce those exact rates.
    cal_mae  = float(np.mean(np.abs(cal_arr - gt_arr)))
    cal_rmse = float(np.sqrt(np.mean((cal_arr - gt_arr) ** 2)))

    # ECE vs actual binary labels: the honest calibration metric.
    ece_label = compute_ece(np.array(ece_y_true), np.array(ece_y_cal))
    if np.isnan(ece_label):
        ece_label = 1.0   # pathological: penalise heavily in H selection

    # H selection: ECE replaces cal_mae so selection is honest.
    selection_score = float(auc_pr - H_SELECTION_ECE_WEIGHT * ece_label)

    return {
        'H':               H,
        'AUC-PR':          auc_pr,
        'AUC-ROC':         auc_roc,
        'Calib_MAE':       cal_mae,      # diagnostic (optimistic)
        'Calib_RMSE':      cal_rmse,     # diagnostic
        'ECE_label':       ece_label,    # honest: vs actual binary labels
...
# every H candidate (cost) for a screening step whose output is only used
...
print(f"  {'H (s)':<8} {'Nested AUC-PR':<16} {'Nested AUC-ROC':<16} {'ECE(label)':<12} {'Cal MAE*':<10} {'Score':<10} {'Grid AUC-PR'}")
print("  " + "-" * 100)
nested_results = []
for H_cand in top_candidates:
    ...
    print(f"  {H_cand:<8} {res['AUC-PR']:<16.4f} {res['AUC-ROC']:<16.4f} "
          f"{res['ECE_label']:<12.4f} {res['Calib_MAE']:<10.4f} "
          f"{res['selection_score']:<10.4f} "
...
print(f"\n  Best H (nested) = {best_H}s  "
      ...
      f"ECE_label={best_result['ECE_label']:.4f}, "
...
nested_summary = pd.DataFrame([{
    ...
    'ECE_label':       r['ECE_label'],      # honest (vs actual binary labels)
    'nested_Cal_MAE':  r['Calib_MAE'],      # diagnostic (optimistic)
...
if len(safe_global) > 0:
    ...
    default_hr_bpm_global            = safe_global['hr_bpm'].median() if 'hr_bpm' in safe_global.columns else 0.5
else:
...
base_f = build_baseline_negatives(
    ...
    default_hr_bpm=default_hr_bpm_global,
)
...
# Categorical check for "Timer" vs "Driver" signals
temporal = ['cognitive_load_decay', 'time_since_last_dist', 'distraction_active']
behavioral = ['model_prob', 'arousal_deviation', 'emotion_prob', 'baseline_error_rate',
              'arousal_delta', 'arousal_at_window_end', 'emotion_changed', 'hr_bpm',
              'model_prob_sq', 'emotion_prob_sq', 'arousal_deviation_sq']
t_imp = importance_df[importance_df['Feature'].isin(temporal)]['Importance'].sum()
b_imp = importance_df[importance_df['Feature'].isin(behavioral)]['Importance'].sum()

print(f"\nInformation Source Balance:")
print(f"  Temporal/Timer Signals: {t_imp:.2%}")
print(f"  Driver/Arousal Signals: {b_imp:.2%}")

# Save the plot
plt.figure(figsize=(10, 6))
plt.barh(importance_df['Feature'], importance_df['Importance'], color='teal')
plt.xlabel('Importance (Gain)')
plt.title(f'Feature Importance: Why is the model predicting risk? (H={best_H}s)')
plt.gca().invert_yaxis()
plt.tight_layout()
plt.savefig(f'{EVAL_OUT}feature_importance.png')
print(f"Saved -> {EVAL_OUT}feature_importance.png")
# ======================================================================
...
cal_X_f  = np.column_stack([raw_all, cld_all, df_f['time_since_last_dist'].values])
final_iso = xgb.XGBRegressor(**CAL_XGB_PARAMS)
final_iso.fit(cal_X_f, gt_all)
...
def predict_fitness(
    ...
    # For inference, we do not have window-based features like arousal_delta, etc.
    # We must set them to safe defaults (0 or median). The model was trained with them,
    # so we must include them with neutral values.
    arousal_delta = 0.0
    arousal_at_window_end = 0.0
    emotion_changed = 0
    hr_bpm = 0.5  # or use a global default; could be stored in artifact
    model_prob_sq = a_prob * a_prob
    emotion_prob_sq = e_prob * e_prob
    arousal_deviation_sq = aro_dev * aro_dev
    ...
    sample = pd.DataFrame([{
        ...
        # NEW FEATURES – set to defaults
        'arousal_delta':           arousal_delta,
        'arousal_at_window_end':   arousal_at_window_end,
        'emotion_changed':         emotion_changed,
        'hr_bpm':                  hr_bpm,
        'model_prob_sq':           model_prob_sq,
        'emotion_prob_sq':         emotion_prob_sq,
        'arousal_deviation_sq':    arousal_deviation_sq,
        # Long-run context
        'baseline_error_rate':    b_rate,
    ...
    # 3-D calibrator: input is [raw_score, cognitive_load_decay, time_since_last_dist]
    cal_input       = np.array([[raw_prob, cld, t_since]])
    calibrated_prob = float(np.clip(art['calibrator'].predict(cal_input)[0], 0.0, 1.0))
