...
Rigorous, publication‑ready pipeline with:
  ...
  - Per‑user arousal baseline as a feature
  - Interactions between driver features and temporal terms
  - Full nested LOSO‑CV with 2‑D calibration
...
EVAL_OUT         = 'evaluation/'
BIN_LIMIT        = 10
H_CANDIDATES     = list(range(9, BIN_LIMIT))
...
# ======================================================================
# Spline transformer for time_since_last_dist (fixed knots)
# ======================================================================
# Knots chosen based on the range of possible times (0 to BIN_LIMIT)
max_time = BIN_LIMIT - 1  # maximum possible time_since in the data (excluding sentinel)
dummy_t = np.linspace(0, max_time, 200).reshape(-1, 1)
spline_transformer = SplineTransformer(
    n_knots=5,                # number of interior knots (can be tuned)
    degree=3,
    include_bias=False,
    extrapolation='constant'
)
spline_transformer.fit(dummy_t)
N_SPLINE_BASIS = spline_transformer.n_features_out_
print(f"Spline basis dimension: {N_SPLINE_BASIS} (knots placed in [0, {max_time}])")
...
# 2‑D calibrator (raw_score, cognitive_load_decay, time_since_last_dist)
CAL_XGB_PARAMS = dict(
    n_estimators       = 200,
    max_depth          = 3,
    learning_rate      = 0.05,
    ...
    verbosity          = 0,
    random_state       = RANDOM_SEED,
    monotone_constraints = (1, 0, 0),   # raw_score ↑, others free
)

# ======================================================================
# Feature list – now includes splines and interactions
# ======================================================================
BASE_FEATURES = [
    # Core temporal state
    'distraction_active',
...
# Spline basis for time_since_last_dist
SPLINE_FEATURES = [f'time_spline_{i}' for i in range(N_SPLINE_BASIS)]
...
AROUSAL_FEATURES = [
    ...
    'emotion_label_enc',
    'arousal_deviation',
    'user_arousal_baseline',          # per‑user median arousal
    'arousal_delta',                   # change during last window
...
# Driver features that will interact with time
DRIVER_FEATURES = [
...
# Build interaction terms: driver × time (linear) and driver × spline
INTERACTION_FEATURES = []
for df in DRIVER_FEATURES:
    INTERACTION_FEATURES.append(f'{df}_x_time')
    for i in range(N_SPLINE_BASIS):
        INTERACTION_FEATURES.append(f'{df}_x_spline_{i}')

FEATURE_COLS = BASE_FEATURES + SPLINE_FEATURES + AROUSAL_FEATURES + CONTEXT_FEATURES + INTERACTION_FEATURES
...
# ======================================================================
# 4. Per‑user arousal baseline (function, recomputed per fold)
# ======================================================================
...
# Global baselines (used only for grid search and final model)
...
# ======================================================================
# 7. Ground truth per‑second error rates
# ======================================================================
...
# ======================================================================
# 11. Sample builders (positives, negatives, baseline)
# ======================================================================
def build_positives(H, user_ids=None, arousal_bl=None, arousal_med=None):
    ...
    for _, err in errors_dist.iterrows():
        ...
        # ==============================================================
        # New features: spline, interactions, etc.
        # ==============================================================
        t_arr = np.array([[time_since]]).reshape(-1, 1)
        spline_vals = spline_transformer.transform(t_arr)[0]  # length N_SPLINE_BASIS
        spline_dict = {f'time_spline_{i}': spline_vals[i] for i in range(N_SPLINE_BASIS)}

        # Driver features (with available values)
        # For positives, we have to derive some from the context (window)
        ...
        if dist_active == 1:
            ...
            # After window – use end values of the previous window
            key = (uid, rid)
            if key in windows_by_session:
                ...
                if not prev.empty:
                    ...
                    # arousal_delta and emotion_changed not defined; keep 0
        ...
        # Driver values for interactions
        driver_vals = {
    ...
    for (uid, rid), wins in windows_by_session.items():
        ...
        for w_idx, win in wins_s.iterrows():
            ...
            for offset in np.arange(0, dur, sample_every):
                ...
                time_since = 0.0
                t_arr = np.array([[time_since]]).reshape(-1, 1)
                spline_vals = spline_transformer.transform(t_arr)[0]
                spline_dict = {f'time_spline_{i}': spline_vals[i] for i in range(N_SPLINE_BASIS)}
        ...
        for i in range(len(wins_s) - 1):
            ...
            for offset in np.arange(0, gap_len, sample_every):
                ...
                t_arr = np.array([[time_since_capped]]).reshape(-1, 1)
                spline_vals = spline_transformer.transform(t_arr)[0]
                spline_dict = {f'time_spline_{i}': spline_vals[i] for i in range(N_SPLINE_BASIS)}
                ...
                interactions = {}
                for df_name, val in driver_vals.items():
                    interactions[f'{df_name}_x_time'] = val * time_since_capped
                    for i in range(N_SPLINE_BASIS):
                        interactions[f'{df_name}_x_spline_{i}'] = val * spline_vals[i]
...
def build_baseline_negatives(sample_every=NEG_SAMPLE_EVERY,
    ...
    for _, row in driving_base.iterrows():
        ...
        b_rate = user_baselines.get(uid, p_baseline_global)
        u_aro = _abl.get(uid, _amed) if default_arousal_baseline is None else default_arousal_baseline
...
# ======================================================================
# 15. Fold‑specific baseline rate (leak‑free)
# ======================================================================
def compute_fold_baseline_rate(user_ids_set):
    ...
    per_user_errs = fold_errs.groupby('user_id').size().reindex(per_user_secs.index, fill_value=0)
...
# ======================================================================
# 17. Core nested LOSO evaluation
# ======================================================================
def full_nested_eval(H):
    ...
    for test_user in all_users:
        ...
        # Calibrator (3‑D: raw_score, cognitive_load_decay, time_since_last_dist)
...
# ======================================================================
# 18. Fast H grid search (no calibration, no baseline)
# ======================================================================
print("\n" + "=" * 70)
...
# ======================================================================
# 19. Full nested evaluation of top candidates
# ======================================================================
print("\n" + "=" * 70)
...
print("  " + "-" * 72)
# Print bin 0 (active) – uncomment if you have total_dist_seconds
...
# ======================================================================
# 26. Final model training (all data)
# ======================================================================
...
base_f = build_baseline_negatives(
    ...
    default_arousal_baseline=global_arousal_median,
)
...
# Categorisation
temporal = ['cognitive_load_decay', 'time_since_last_dist', 'distraction_active'] + SPLINE_FEATURES
behavioral = ['model_prob', 'arousal_deviation', 'emotion_prob', 'baseline_error_rate',
              ...
              'user_arousal_baseline'] + [f for f in FEATURE_COLS if '_x_' in f]
...
filtered = importance_df[importance_df["Importance"] >= 0.01]
plt.barh(filtered["Feature"], filtered["Importance"], color="teal")
...
def predict_fitness(
    ...
    t_arr = np.array([[t_since]]).reshape(-1, 1)
    spline_vals = spline_trans.transform(t_arr)[0]
    n_spline = art['n_spline_basis']
    spline_dict = {f'time_spline_{i}': spline_vals[i] for i in range(n_spline)}

    # Driver features – defaults for those not provided
    arousal_delta = 0.0
    hr_bpm = 0.5
    emotion_changed = 0
    arousal_at_window_end = 0.0
    ...
    # Quadratic terms
    m_prob_sq = a_prob * a_prob
    e_prob_sq = e_prob * e_prob
    aro_dev_sq = aro_dev * aro_dev
    ...
    interactions = {}
    for df_name, val in driver_vals.items():
        interactions[f'{df_name}_x_time'] = val * t_since
