...
Changes from ext5:
  - Removed WeightedRandomSampler: redundant after SMOTE has balanced classes to ~50/50.
    Upweighting positives a second time pushed the effective positive rate above 50% in
    each batch, causing the model to over-rotate toward positives.
  - Replaced Focal loss (γ=2) + pos_weight with plain BCE: after SMOTE the classes are
    balanced so pos_weight=1.0 is correct.  Focal loss focuses on hard (often noisy)
    examples, amplifying label noise in the distraction-error association.
  - Training loader now uses shuffle=True always (no sampler).
...
# Spectral features — kinematics only (4 signals × 3 bands = 12-d)
SPECTRAL_BANDS = [(0.0, 0.1), (0.1, 0.3), (0.3, 0.5)]
KIN_SPECTRAL_DIM = len(KIN_COLS) * len(SPECTRAL_BANDS)   # 4 × 3 = 12
...
def train_kin_tcn(model, Xtr_k, Xtr_ps, Xtr_s, y_tr,
    ...
    Loss: plain BCE. After SMOTE the training set is balanced (~50/50), so
    pos_weight=1.0 is correct and no additional reweighting is needed.
...
def main():
    ...
    print(f"Head input        : {KinTCN.KIN_D} + {PHYS_SCALAR_D} + {KIN_SPECTRAL_DIM} = {KinTCN.KIN_D + PHYS_SCALAR_D + KIN_SPECTRAL_DIM}-d")
    ...
    for d in drivers:
        ...
        # ── Spectral features (kinematics only) ───────────────────────────────────
        Xtr_s_real  = compute_spectral_features_batch(X_tr_train[:n_real, :, KIN_IDX],
                                                      sample_dt=global_sample_dt)
        Xval_s_feat = compute_spectral_features_batch(X_val_raw[:, :, KIN_IDX],
                                                      sample_dt=global_sample_dt)
        Xte_s_feat  = compute_spectral_features_batch(X_te[:, :, KIN_IDX],
                                                      sample_dt=global_sample_dt)
        scaler_s = StandardScaler()
        scaler_s.fit(Xtr_s_real)
        Xtr_s_real_sc = scaler_s.transform(Xtr_s_real).astype(np.float32)
        Xval_s_sc     = scaler_s.transform(Xval_s_feat).astype(np.float32)
        Xte_s_sc      = scaler_s.transform(Xte_s_feat).astype(np.float32)

        if n_synthetic > 0:
            lams_s = _lambdas[:, None]
            Xtr_s_syn_sc = ((1.0 - lams_s) * Xtr_s_real_sc[_anchors]
                            + lams_s         * Xtr_s_real_sc[_neighbors])
    ...
    out_path = OUT_DIR / "kin_tcn_results.json"
