...
# Added back the vehicle telemetry signals
SIGNAL_COLS = [
    "arousal",
    "hr",
    "speed.x",
...
class FocalLoss(nn.Module):
    # Tuned alpha down to 0.10 to better match the ~10% positive rate
    def __init__(self, alpha=0.10, gamma=2.0):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
...
class TemporalCNN(nn.Module):
    def __init__(self, in_channels):
        ...
        self.net = nn.Sequential(
            ...
            nn.BatchNorm1d(128),
            nn.ReLU(),
            # Removed AdaptiveAvgPool1d to preserve sequence length for the GRU
        )
    ...
    def forward(self, x):
        ...
        z = z.permute(0, 2, 1)        # [B, L, 128] for GRU
        out, hidden = self.gru(z)     # hidden is [1, B, 64]
...
def train_model(X_tr, y_tr):
    ...
    criterion = FocalLoss(alpha=0.10, gamma=2.0)
...
def lopo_baseline(X, y, pid, eligible):
    Xf = build_stat_features(X)
    results = {}
    # Only evaluate on eligible drivers, but train on ALL others
    for p in eligible:
        tr, te      = pid != p, pid == p
...
def lopo_cnn(X, y, pid, eligible):
    ...
    for p in eligible:
        ...
        # --- personalization (Stratified) ---
        try:
            X_calib, X_eval, y_calib, y_eval = train_test_split(
                X_te, y_te, test_size=1-CALIB_FRAC, stratify=y_te, random_state=42
            )
        except ValueError:
            # Fallback if there are too few samples to stratify properly
            n_calib = max(1, int(len(X_te) * CALIB_FRAC))
            X_calib, y_calib = X_te[:n_calib], y_te[:n_calib]
            X_eval,  y_eval  = X_te[n_calib:], y_te[n_calib:]

        if y_eval.sum() == 0 or y_calib.sum() == 0:
...
def compare_results(baseline, population, personalized):
    ...
    def mean_std(res, key):
        ...
        return np.mean(vals), np.std(vals) if len(vals) > 0 else 0
...
# ----------------------------
# Autoencoder Upgraded
# ----------------------------

class DrivingAutoencoder(nn.Module):
    def __init__(self, in_channels, seq_len=60):
        super().__init__()
    ...
    def reconstruction_error(self, x):
        ...
        # Add gradient-based error to focus on anomalies rather than baseline drift
        mse_raw = ((x - recon) ** 2).mean(dim=(1, 2))
        diff_x = torch.diff(x, dim=1)
        diff_recon = torch.diff(recon, dim=1)
        mse_diff = ((diff_x - diff_recon) ** 2).mean(dim=(1, 2))
        return mse_raw + mse_diff

def train_autoencoder(X_tr_normal):
    dataset = DrivingDataset(X_tr_normal, np.zeros(len(X_tr_normal)))
    ...
    for epoch in range(30):
        model.train()
        pbar = tqdm(loader, desc=f"AE Epoch {epoch+1}", leave=False)
        for x, _ in pbar:
            x = x.to(DEVICE)
            recon = model(x)
            # Use standard MSE for training, but gradient-aware error for inference
...
def evaluate_autoencoder(model, X_te):
    model.eval()
    scores = []
    with torch.no_grad():
        for i in range(0, len(X_te), 512):
...
def lopo_autoencoder(X, y, pid, eligible):
    results = {}
    for p in eligible:
        tr, te     = pid != p, pid == p
        X_tr, y_tr = X[tr], y[tr]
        ...
        model       = train_autoencoder(X_tr_normal)
        scores      = evaluate_autoencoder(model, X_te)
        ...
        print(f"[AE] Driver {p} "
              f"AUC {results[p]['auc']:.3f} "
              f"AP  {results[p]['ap']:.3f}")

    aucs = [v["auc"] for v in results.values()]
    aps  = [v["ap"]  for v in results.values()]
    print(f"\nAE LOPO AUROC: {np.mean(aucs):.3f} ± {np.std(aucs):.3f}")
    print(f"AE LOPO AP:    {np.mean(aps):.3f} ± {np.std(aps):.3f}")
    return results
...
def main():
    ...
    # Get eligibility list, but DO NOT drop data. 
    # Keep the entire dataset (X, y, pid) intact for training!
    eligible, _ = driver_eligibility_report(X, y, pid)
    
    # Normalize per driver across the FULL dataset
    Xn = normalize_per_driver(X, pid)

    # Pass the 'eligible' list so it only tests on eligible drivers
    print("\nRunning baseline...")
    baseline_res = lopo_baseline(Xn, y, pid, eligible)

    print("\nRunning CNN (population + personalization)...")
    pop_res, pers_res = lopo_cnn(Xn, y, pid, eligible)

    print("\nRunning Autoencoder...")
    lopo_autoencoder(Xn, y, pid, eligible)
